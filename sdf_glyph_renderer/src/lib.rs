...
//! with font files when the optional `font` feature is enabled.
//!
...
//!
//! Rather than re-invent the rasterisation process for fonts, this crate relies on a Rust font
//! renderer to generate the bitmap. This is quite fast (we're talking µs/glyph), and the results are
...
//! SDF glyphs from source fonts. If you're looking for a batch generation tool,
